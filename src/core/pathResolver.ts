...
import { PathResolveResult, VSCodePathVariables } from '../types';
...
export class VSCodePathResolver {
   ...
   * 解析VSCode原生路径变量
   */
  resolveVariables(pattern: string, markdownFilePath: string, imageFileName: string): string {
   ...
   * 获取VSCode路径变量
   */
  private getPathVariables(markdownFilePath: string, imageFileName: string): VSCodePathVariables {
   ...
   */
  getDefaultSavePath(markdownFilePath: string, imageFileName: string): PathResolveResult {
   ...
   * 生成用于文件监控的glob模式
   */
  generateWatchPatterns(destinationConfig?: Record<string, string>): string[] {
    if (!destinationConfig) {
      // 未配置时，监控所有工作区中的图片文件
  ...
  findRelatedMarkdownFile(
    ...
    const possibleMarkdownFiles: string[] = [];
