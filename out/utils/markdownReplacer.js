...
class MarkdownReplacer {
     ...
     */
    async replaceImageLink(markdownFilePath, localImagePath, remoteUrl) {
        try {
            // 打开或获取文档
     ...
     * 在文档中查找图片引用
     */
    findImageReference(document, localImagePath) {
     ...
     * 查找第一个匹配项
     */
    findFirstMatch(document, text, regex, localImagePath) {
        let match;
        const matches = [];
        ...
        return {
            ...
            newText: (remoteUrl) => `![${match_result[1] || ''}](${remoteUrl})`
        };
    }
    /**
     * 查找最近的图片引用（模糊匹配）
     */
    findRecentImageReference(document, text, _imageFileName) {
        // 查找所有图片引用
        const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
        let match;
        const allMatches = [];
     ...
     * 计算路径匹配分数
     */
    calculateMatchScore(imagePath, localImagePath) {
        let score = 0;
        const imageFileName = path.basename(localImagePath);
     ...
     * 获取光标应该定位的位置
     */
    getCursorPosition(document, line, column) {
        // 使用通用的安全计算函数
        return (0, cursorPosition_1.createSafePosition)(document, line, column);
    }
}
exports.MarkdownReplacer = MarkdownReplacer;
//# sourceMappingURL=markdownReplacer.js.map