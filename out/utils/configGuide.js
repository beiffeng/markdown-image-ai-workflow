...
exports.ConfigurationGuide = void 0;
...
2. 选择并配置了图床服务
`;
        const choice = await vscode.window.showInformationMessage(message, '立即配置', '查看文档', '稍后');
        switch (choice) {
            case '立即配置':
     ...
     * 检查并配置VSCode
     */
    async checkAndConfigureVSCode() {
        const config = vscode.workspace.getConfiguration();
        const destination = config.get('markdown.copyFiles.destination');
        if (!destination) {
            const choice = await vscode.window.showInformationMessage('需要先配置VSCode的图片保存位置。是否使用推荐配置？', '使用推荐配置', '手动配置', '取消');
            if (choice === '使用推荐配置') {
                try {
                    await config.update('markdown.copyFiles.destination', { '**/*.md': 'assets/${documentBaseName}/' }, vscode.ConfigurationTarget.Workspace);
                    vscode.window.showInformationMessage('✅ VSCode配置已设置：图片将保存到 assets/{文档名}/ 目录');
                    return true;
                }
                catch (error) {
     ...
     */
    async configureSMMS() {
        const choice = await vscode.window.showInformationMessage('SM.MS无需配置即可使用。是否要配置API Token以获得更高的上传限制？', '配置Token', '跳过');
        if (choice === '配置Token') {
            const token = await vscode.window.showInputBox({
                title: '配置SM.MS Token',
     ...
     * 显示配置完成
     */
    async showConfigurationComplete() {
        const message = `
✅ 配置完成！
...
`;
        const choice = await vscode.window.showInformationMessage(message, '开始使用', '查看设置');
        if (choice === '查看设置') {
     ...
     * 解释VSCode功能
     */
    async showVSCodeFeatureExplanation() {
        const explanation = `
...
exports.ConfigurationGuide = ConfigurationGuide;
//# sourceMappingURL=configGuide.js.map