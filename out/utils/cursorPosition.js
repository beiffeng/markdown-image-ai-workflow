...
exports.CursorPositioner = void 0;
exports.clampLineNumber = clampLineNumber;
exports.clampColumnNumber = clampColumnNumber;
exports.createSafePosition = createSafePosition;
const vscode = require("vscode");
/**
 * 文档位置安全计算工具函数
 */
function clampLineNumber(line, document) {
    return Math.max(0, Math.min(line, document.lineCount - 1));
}
function clampColumnNumber(column, document, line) {
    const safeLine = clampLineNumber(line, document);
    const lineText = document.lineAt(safeLine).text;
    return Math.max(0, Math.min(column, lineText.length));
}
function createSafePosition(document, line, column) {
    const safeLine = clampLineNumber(line, document);
...
class CursorPositioner {
     ...
     * 将光标定位到指定文件的指定位置
     */
    async positionCursor(filePath, line, column, options = {}) {
        try {
            // 默认选项
            const { reveal = true, focus = true, selection = false } = options;
            // 打开或激活文档
     ...
     * 将光标定位到行尾
     */
    async positionToLineEnd(filePath, line, options) {
        try {
            const document = await this.getOrOpenDocument(filePath);
            if (!document) {
                return false;
     ...
     * 定位到替换后的图片链接末尾
     */
    async positionAfterImageReplacement(filePath, line, remoteUrl, altText) {
        try {
            const document = await this.getOrOpenDocument(filePath);
            if (!document) {
                return false;
     ...
     * 显示成功提示并定位光标
     */
    async showSuccessAndPosition(filePath, line, column, message) {
        // 显示成功提示
     ...
     * 获取或打开文档
     */
    async getOrOpenDocument(filePath) {
        try {
            // 检查是否已打开
     ...
     * 显示文本文档
     */
    async showTextDocument(document, focus = true) {
        try {
            const options = {
                preserveFocus: !focus,
                preview: false
     ...
     * 计算安全的位置（不超出文档范围）
     */
    calculateSafePosition(document, line, column) {
        // 使用通用的安全计算函数
     ...
     * 检查指定文件是否为当前活动文档
     */
    isActiveDocument(filePath) {
        const editor = vscode.window.activeTextEditor;
        return editor?.document.fileName === filePath;
    }
}
exports.CursorPositioner = CursorPositioner;
